...
# Dados em formato de string JSON

# Carregar os dados usando json.loads()
...
#Posicionando botoes de start e exit
botao_start = Botao(largura//2 - 50  , altura//2 + 200, start_img)
botao_sair = Botao(largura//2 - 50 , altura//2 + 260,exit_img)

nave = Nave(int(largura / 2), altura - 100, 5)  # Cria uma instância da classe Nave 
nave_grupo.add(nave)  # Adiciona a nave ao grupo de naves do jogador

run = True  # Variável de controle do loop principal do jogo

#Caixas para digitar Usuario e Senha
...
'''def verificar_usuario_existente(nome_usuario, dados_existentes):
...
while run:  # Loop principal do jogo
    ...
    if menu == True:
        ...
        draw_text("Usuario : ",fonte30,branco,175,455)#aqui
        draw_text("Senha : ",fonte30,branco,197 ,505)#aqui
                        ...
                        for jogador in dados_existentes["jogadores"]:
                            if isinstance(jogador, dict) and jogador.get("nome") == nome_usuario:
                                if "senha" in jogador and jogador["senha"] == senha:
                                    ...
                                    draw_texto("Senha incorreta. Tente novamente.", fonte15, vermelho, 180, 400)
                                    break
                    else:
                        print(f"Usuário {nome_usuario} não existe na lista. Adicionando como novo jogador.")
                        novo_jogador = True
                        menu = False
                else:
                    draw_texto("Digite um nome de usuário válido.", fonte15, vermelho, 180, 400)
    else:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if input_rect.collidepoint(event.pos) and ativa_senha == False:
                ativa_user = True
            else:
                ativa_user = False
        if event.type == pygame.KEYDOWN:
            if ativa_user == True and ativa_senha == False:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if senha_rect.collidepoint(event.pos) and ativa_user == False:
                ativa_senha = True
            else:
                ativa_senha = False
        if event.type == pygame.KEYDOWN:
            if ativa_senha == True and ativa_user == False:   
                if event.key == pygame.K_BACKSPACE:
                    senha = senha[0:-1]
                else:
                   senha +=  event.unicode
...
#fecha o pygame
pygame.quit()
# Atualização e salvamento dos dados
dados_jogo = {
    "nome": nome_usuario,
    "pontos": pontos,
    "senha": senha
